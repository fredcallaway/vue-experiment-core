...
- Prolific review action and bonus overrides now sync through `studies/<studyId>/reviewDraft` in RTDB instead of browser-local storage. The review page applies per-submission and per-participant updates so browser and agent edits to different entries do not overwrite each other, and the final Execute confirmation uses the exact state shown when the modal opened.
