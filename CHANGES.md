...
- A Prolific study reported as `COMPLETED` now remains `AWAITING REVIEW` in the dashboard while any participant has an unpaid intended bonus. The detail page, study table, and next-study summary switch to `COMPLETED` only after every non-zero intended bonus has been fully paid.
